...
fn parse_arguments() -> Result<Config, String> {
        ...
        match argument.as_str() {
            "--input" | "-i" => {
                input_path = Some(PathBuf::from(
                    next_argument(&mut args, "--input")?
                ));
            }

            "--output" | "-o" => {
                output_path = Some(PathBuf::from(
                    next_argument(&mut args, "--output")?
                ));
            }
            ...
            "--vocab-size" | "-v" => {
                ...
                vocab_size = value.parse::<usize>().map_err(|_| {
                    format!("Invalid vocabulary size: {value}")
                })?;
            }
            ...
            "--min-frequency" | "-f" => {
                ...
                min_frequency = value.parse::<u64>().map_err(|_| {
                    format!("Invalid minimum frequency: {value}")
                })?;
            }
    ...
    if vocab_size < 4 {
        return Err(
            "Vocabulary size must be at least 4 for the special tokens."
                .to_string(),
        );
    }
